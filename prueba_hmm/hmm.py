...
class hmm:
	...
	def add_obserbator(self, hidden_state, random_variable):
		if not self.state_transition.has_key(state):
			raise Exception("No esta el hidden state " + str(hidden_state))
	
		self.state_observation[hidden_state]= random_variable
...
class random_observation:
	...
	def next(self):
		...
		for state, prob in hmm.nexts(self.actual_state).items():
			rnd-= prob
			if rnd < 0:
		...
		res= {}
		for random_variable in hmm.observations(self.actual_state):
