...
class MidiScoreWriter(ScoreWriter):
    ...
    def _get_fading_tick(self, score):
    ...
    def _fade_score(self, score):
        ...
        for instrument, notes in score.notes_per_instrument.iteritems():
            ...
            for n in fading_notes:
                ...
                a= float((n.start - start_fading_tick)/(fading_time - score.seconds2ticks(1)))
                n.volume= max(0, int(n.volume - a*n.volume))
                if n.volume < 20: to_remove.append(n)

            notes= [n for n in notes if n not in to_remove]
            score.notes_per_instrument[instrument]= notes
        return score
