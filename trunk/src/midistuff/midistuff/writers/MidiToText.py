...
class MidiToText(MidiOutStream):
    ...
    def note_on(self, channel=0, note=0x40, velocity=0x40):
        print 'note_on  - ch:%02X,  note:%02X,  vel:%02X time:%s' % (int(channel), int(note), int(velocity), self.abs_time())

    def note_off(self, channel=0, note=0x40, velocity=0x40):
        print 'note_off - ch:%02X,  note:%02X,  vel:%02X time:%s' % (int(channel), int(note), int(velocity), self.abs_time())
